...
public class MySqlStatusRepository extends MySqlRepository implements IStatusRepository {
	...
	private void validateStatus (Status status) {
		if (status.getDescription() == null ||
			...
			throw new IllegalArgumentException(
					"O campo 'descrição' do status não pode ser nulo.");
		}
		...
		if (status.getpKStatus() == null || status.getpKStatus().trim().isEmpty()) {
			...
			if (status.getpKStatus().length() != 36) {
				throw new IllegalArgumentException(
						"A chave primária do status deve ser um UUID.");
			}
	...
	public void insertStatus(Status status) {
		...
		SqlParameterSource namedParameter = new BeanPropertySqlParameterSource(status);
	...
	public Status getStatusByDescription(String description) {
		...
		SqlParameterSource parameters = new MapSqlParameterSource ("descrption", description);
		
		List <Status> result = jdbcTemplate.query(query, parameters, new
				RowMapper <Status> () {
